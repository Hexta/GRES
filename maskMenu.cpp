...
{
	QString selFilter = "";
	QString fileName = QFileDialog::getOpenFileName( this, tr( "Open file with mask" ), "./masks/", tr( "Images (*.bmp *.gif *.png *.tiff *.jpg *.xpm)" ), &selFilter );
	if ( !fileName.isNull( ) )//проверяем не отменил ли юзер выбор файла
	{
		maskImage = new QImage( fileName );
		if ( maskImage->isNull( ) )
		{
			QMessageBox::information( this, tr( "GRES" ), tr( "Cannot load %1." ).arg( fileName ) );
			return;
		}
